...
def parse_a3m_file(a3m_file):
    """Parse A3M MSA file and return alignment string"""
    if not os.path.exists(a3m_file):
        print(f"Warning: MSA file '{a3m_file}' not found")
        return None
    
    with open(a3m_file, 'r') as f:
        alignment_content = f.read().strip()
    
    print(f"Loaded MSA from {a3m_file}")
    print(f"MSA content length: {len(alignment_content)} characters")
...
def make_prediction_request(sequence, output_format="mmcif", chain_id="A", msa_content=None):
    """Make a prediction request to Boltz2 with optional MSA"""
    headers = {
        "content-type": "application/json"
    }
    # Format 5: MSA with nested dictionary structure in polymer
    polymer_def = {
        "id": chain_id,
    ...
    if msa_content:
        ...
        print("Including MSA data (format 7: all lowercase)")
    
    data = {
        "polymers": [polymer_def],
        "recycling_steps": 3,
        "sampling_steps": 50,
        "diffusion_samples": 1,
        "step_scale": 1.638,
...
def main():
    ...
    # Add MSA argument
    parser.add_argument('-m', '--msa', type=str, help='Path to A3M MSA file (optional)', default=None)
    
    args = parser.parse_args()
    
    # Load MSA if provided
    msa_content = None
    if args.msa:
        msa_content = parse_a3m_file(args.msa)
        if not msa_content:
            print(f"Warning: Could not load MSA file '{args.msa}', proceeding without MSA")
    ...
    if args.fasta:
        ...
        if msa_content:
            print(f"Using MSA file: {args.msa}")
        ...
        for seq_info in sequences:
            ...
            # Make prediction with MSA if available
            result = make_prediction_request(sequence, "mmcif", chain_id, msa_content)
        ...
        # Make prediction with MSA if available
        result = make_prediction_request(sequence, "mmcif", "A", msa_content)
